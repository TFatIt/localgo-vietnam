  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (!GoogleSignin?.signIn) {
        Alert.alert(
          'Chế độ Expo Go',
          'Đăng nhập Google yêu cầu bản build độc lập (EAS Build). Trên Expo Go, vui lòng chọn "Tiếp tục với tư cách khách" hoặc "Đăng nhập Email".'
        );
        return;
      }
      ...
      if ((error as { code?: string }).code !== 'SIGN_IN_CANCELLED') {
        Alert.alert('Google Sign-In thất bại', 'Vui lòng thử lại hoặc dùng chế độ Khách.');
      }
    } finally {
      setLoading(null);
    }
